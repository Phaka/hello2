...
light.exe -out bin\Release\hello-%RUNTIME%.msi -pdbout obj\Release\hello.wixpdb -cultures:null -ext "%WIX%\bin\WixUIExtension.dll" obj\Release\Product.wixobj obj\Release\HelloPortal.wixobj

candle.exe -out obj\Release\ -dMsiPackagePath=bin\Release\hello-%RUNTIME%.msi -dVersion=%VERSION% -arch x64 -ext "%WIX%\bin\WixBalExtension.dll" Bundle.wxs
light.exe -out bin\Release\hello_%VERSION%-%RUNTIME%.exe -pdbout obj\Release\Bootstrapper.wixpdb -ext "%WIX%\bin\WixBalExtension.dll" obj\Release\Bundle.wixobj
popd 
